/*		FADEC Code - FailSafeFunctions.c 
 * % *************************************************************************
                  ...
                  double Emergency, double EmergencyWord,double Fail, double FailWord)
{
    ...
    /* If enought time has ellapsed since RPM > 0 has been detected, windmilling must be indicated */
    else if (Emergency == EmergencyWord)
    {
        /* updates status */
        EngStatusP->EngineStatus = 0;
                    ...
                    double Fail, double FailWord, double Ts)
{
    ...
    case 1:
        ...
        WindMillingState(EngStatusP, RPM, ContConstP, &TimerWindMill, TimeWindMill, status, Ts);
    ...
    case 8:
        ...
        RPMIdleState(EngStatusP, RPM, status, statusIdle);
...
{
    /* If ResModel > ResComp, there is a residual compare between ResModel and ResComp*/
    if((ResModel > ResComp) | ((*CounterP)>0))
    {
 ...
 *
 *	PURPOSE:		Verifies if hard fault (high bias) has occured
 *
 *	CALLED FROM:	tbd
 *
 *	PARAMETERS:		FaultDetect *TrustSensP, SensorFaults *FaultSSensP
...
{
    /* Ts2 thermocouples */
    FaultStatusP->Faults.Ts2_0_Bias = FaultHardP->DetFaults.Ts2_0_Fault;
    FaultStatusP->Faults.Ts2_1_Bias = FaultHardP->DetFaults.Ts2_1_Fault;    

    /* Pt3 sensor */
    FaultStatusP->Faults.Pt3_0_Bias = FaultHardP->DetFaults.Pt3_0_Fault; 

    /* Tt4 thermocouples */
    FaultStatusP->Faults.Tt4_0_Bias = FaultHardP->DetFaults.Tt4_0_Fault;
    FaultStatusP->Faults.Tt4_1_Bias = FaultHardP->DetFaults.Tt4_1_Fault; 
    FaultStatusP->Faults.Tt4_2_Bias = FaultHardP->DetFaults.Tt4_2_Fault;

    /* RPM thermocouples */
    FaultStatusP->Faults.RPM_0_Bias = FaultHardP->DetFaults.RPM_0_Fault;
    FaultStatusP->Faults.RPM_1_Bias = FaultHardP->DetFaults.RPM_1_Fault; 

    /* POil sensor */
    FaultStatusP->Faults.POil_0_Bias = FaultHardP->DetFaults.POil_0_Fault; 

    /* PFuel sensor */
    FaultStatusP->Faults.PFuel_0_Bias = FaultHardP->DetFaults.PFuel_0_Fault; 

    /* Vib sensor */
    FaultStatusP->Faults.Vib_0_Bias = FaultHardP->DetFaults.Vib_0_Fault; 
 ...
 *
 *	PURPOSE:		Updates detected sensor faults
 *
 *	CALLED FROM:	tbd
 *
 *	PARAMETERS:		FaultDetect *TrustSensP, SensorFaults *FaultSSensP
...
{
    /* Ts2 0 thermocouple */
    FaultStatusP->Faults.Ts2_0_SCV = FaultSensP->SensFaults.Ts2_0_SCV;
    FaultStatusP->Faults.Ts2_0_SCG = FaultSensP->SensFaults.Ts2_0_SCG;
    FaultStatusP->Faults.Ts2_0_Open = FaultSensP->SensFaults.Ts2_0_Open;

    /* Ts2 1 thermocouple */
    FaultStatusP->Faults.Ts2_1_SCV = FaultSensP->SensFaults.Ts2_1_SCV;
    FaultStatusP->Faults.Ts2_1_SCG = FaultSensP->SensFaults.Ts2_1_SCG;
    FaultStatusP->Faults.Ts2_1_Open = FaultSensP->SensFaults.Ts2_1_Open;

    /* Pt3 0 sensor */
    FaultStatusP->Faults.Pt3_0_Short = FaultSensP->SensFaults.Pt3_0_Short;
    FaultStatusP->Faults.Pt3_0_Open = FaultSensP->SensFaults.Pt3_0_Open;
    
    /* Tt4 0 thermocouple */
    FaultStatusP->Faults.Tt4_0_SCV = FaultSensP->SensFaults.Tt4_0_SCV;
    FaultStatusP->Faults.Tt4_0_SCG = FaultSensP->SensFaults.Tt4_0_SCG;
    FaultStatusP->Faults.Tt4_0_Open = FaultSensP->SensFaults.Tt4_0_Open;

    /* Tt4 1 thermocouple */
    FaultStatusP->Faults.Tt4_1_SCV = FaultSensP->SensFaults.Tt4_1_SCV;
    FaultStatusP->Faults.Tt4_1_SCG = FaultSensP->SensFaults.Tt4_1_SCG;
    FaultStatusP->Faults.Tt4_1_Open = FaultSensP->SensFaults.Tt4_1_Open;

    /* Tt4 2 thermocouple */
    FaultStatusP->Faults.Tt4_2_SCV = FaultSensP->SensFaults.Tt4_2_SCV;
    FaultStatusP->Faults.Tt4_2_SCG = FaultSensP->SensFaults.Tt4_2_SCG;
    FaultStatusP->Faults.Tt4_2_Open = FaultSensP->SensFaults.Tt4_2_Open;

    /* RPM 0 thermocouple */
    FaultStatusP->Faults.RPM_0_Short = FaultSensP->SensFaults.RPM_0_Short;
    FaultStatusP->Faults.RPM_0_Open = FaultSensP->SensFaults.RPM_0_Open;
    FaultStatusP->Faults.RPM_0_1PLoss = FaultSensP->SensFaults.RPM_0_PLoss1;
    FaultStatusP->Faults.RPM_0_2PLoss = FaultSensP->SensFaults.RPM_0_PLoss2;

    /* RPM 1 thermocouple */
    FaultStatusP->Faults.RPM_1_Short = FaultSensP->SensFaults.RPM_1_Short;
    FaultStatusP->Faults.RPM_1_Open = FaultSensP->SensFaults.RPM_1_Open;
    FaultStatusP->Faults.RPM_1_1PLoss = FaultSensP->SensFaults.RPM_1_PLoss1;
    FaultStatusP->Faults.RPM_1_2PLoss = FaultSensP->SensFaults.RPM_1_PLoss2;

    /* POil 0 sensor */
    FaultStatusP->Faults.POil_0_Short = FaultSensP->SensFaults.POil_0_Short;
    FaultStatusP->Faults.POil_0_Open = FaultSensP->SensFaults.POil_0_Open;

    /* PFuel 0 sensor */
    FaultStatusP->Faults.PFuel_0_Short = FaultSensP->SensFaults.PFuel_0_Short;
    FaultStatusP->Faults.PFuel_0_Open = FaultSensP->SensFaults.PFuel_0_Open;

    /* Vib 0 sensor */
    FaultStatusP->Faults.Vib_0_Short = FaultSensP->SensFaults.Vib_0_Short;
    FaultStatusP->Faults.Vib_0_Open = FaultSensP->SensFaults.Vib_0_Open;

}

/*******************************************************************************
 *
 *	FUNCTION:		SysFaultUpdt()
 *
                 ...
                 double *MapHPC1DP, double *MapHPC1DHP, double *MapHPC2DP, double *MapHPC2DHP)
{
    ...
    //mexPrintf("Delta = %f, Theta = %f, Tt2 = %f\n", Delta, Theta, Tt2);    
...
//mexPrintf("Tt4 = %f + %f * (%f-%f) = ", Tt4ss, Tt4D, Wf, Wfss);
...
void PrintFaults(EngineFaults *FaultStatusP, int WhichPrint, int tUpdt)
{
    ...
    if(FaultStatusP->FaultBits != 0)
    {
        ...
        /* TS2 0 Faults */
        if(FaultStatusP->Faults.Ts2_0_Bias){ mexPrintf("\n Ts2 0 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.Ts2_0_SCV){ mexPrintf("\n Ts2 0 - SCV - at t = %f",t); }
        if(FaultStatusP->Faults.Ts2_0_SCG){ mexPrintf("\n Ts2 0 - SCG - at t = %f",t); }
        if(FaultStatusP->Faults.Ts2_0_Open){ mexPrintf("\n Ts2 0 - Open - at t = %f",t); }

        /*TS2 1 Faults*/
        if(FaultStatusP->Faults.Ts2_1_Bias){ mexPrintf("\n Ts2 1 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.Ts2_1_SCV){ mexPrintf("\n Ts2 1 - SCV - at t = %f",t); }
        if(FaultStatusP->Faults.Ts2_1_SCG){ mexPrintf("\n Ts2 1 - SCG - at t = %f",t); }
        if(FaultStatusP->Faults.Ts2_1_Open){ mexPrintf("\n Ts2 1 - Open - at t = %f",t); }

        /*Pt3 0 Faults*/
        if(FaultStatusP->Faults.Pt3_0_Bias){ mexPrintf("\n Pt3 0 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.Pt3_0_Open){ mexPrintf("\n Pt3 0 - Open - at t = %f",t); }
        if(FaultStatusP->Faults.Pt3_0_Short){ mexPrintf("\n Pt3 0 - Short - at t = %f",t); }

        /* Tt4 0 Faults */
        if(FaultStatusP->Faults.Tt4_0_Bias){ mexPrintf("\n Tt4 0 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_0_SCV){ mexPrintf("\n Tt4 0 - SCV - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_0_SCG){ mexPrintf("\n Tt4 0 - SCG - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_0_Open){ mexPrintf("\n Tt4 0 - Open - at t = %f",t); }

        /* Tt4 1 Faults */
        if(FaultStatusP->Faults.Tt4_1_Bias){ mexPrintf("\n Tt4 1 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_1_SCV){ mexPrintf("\n Tt4 1 - SCV - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_1_SCG){ mexPrintf("\n Tt4 1 - SCG - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_1_Open){ mexPrintf("\n Tt4 1 - Open - at t = %f",t); }

        /* Tt4 2 Faults */
        if(FaultStatusP->Faults.Tt4_2_Bias){ mexPrintf("\n Tt4 2 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_2_SCV){ mexPrintf("\n Tt4 2 - SCV - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_2_SCG){ mexPrintf("\n Tt4 2 - SCG - at t = %f",t); }
        if(FaultStatusP->Faults.Tt4_2_Open){ mexPrintf("\n Tt4 2 - Open - at t = %f",t); }

        /* RPM 0 Faults */
        if(FaultStatusP->Faults.RPM_0_Bias){ mexPrintf("\n RPM 0 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_0_Short){ mexPrintf("\n RPM 0 - Short - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_0_Open){ mexPrintf("\n RPM 0 - Open - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_0_1PLoss){ mexPrintf("\n RPM 0 - 1 Pulse Loss - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_0_2PLoss){ mexPrintf("\n RPM 0 - 2 Pulses Loss - at t = %f",t); }

        /* RPM 1 Faults */
        if(FaultStatusP->Faults.RPM_1_Bias){ mexPrintf("\n RPM 1 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_1_Short){ mexPrintf("\n RPM 1 - Short - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_1_Open){ mexPrintf("\n RPM 1 - Open - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_1_1PLoss){ mexPrintf("\n RPM 1 - 1 Pulse Loss - at t = %f",t); }
        if(FaultStatusP->Faults.RPM_1_2PLoss){ mexPrintf("\n RPM 1 - 2 Pulses Loss - at t = %f",t); }

        /* POil 0 Faults */
        if(FaultStatusP->Faults.POil_0_Bias){ mexPrintf("\n POil 0 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.POil_0_Open){ mexPrintf("\n POil 0 - Open - at t = %f",t); }
        if(FaultStatusP->Faults.POil_0_Short){ mexPrintf("\n POil 0 - Short - at t = %f",t); }

        /* PFuel 0 Faults */
        if(FaultStatusP->Faults.PFuel_0_Bias){ mexPrintf("\n PFuel 0 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.PFuel_0_Open){ mexPrintf("\n PFuel 0 - Open - at t = %f",t); }
        if(FaultStatusP->Faults.PFuel_0_Short){ mexPrintf("\n PFuel 0 - Short - at t = %f",t); }

        /* Vibration 0 Faults */
        if(FaultStatusP->Faults.Vib_0_Bias){ mexPrintf("\n Vib 0 - Bias - at t = %f",t); }
        if(FaultStatusP->Faults.Vib_0_Open){ mexPrintf("\n Vib 0 - Open - at t = %f",t); }
        if(FaultStatusP->Faults.Vib_0_Short){ mexPrintf("\n Vib 0 - Short - at t = %f",t); }

        /* Lubrication System */
        if(FaultStatusP->Faults.POil_High){ mexPrintf("\n POil High - at t = %f",t); }
        if(FaultStatusP->Faults.POil_Low){ mexPrintf("\n POil Low - at t = %f",t); }

        /* Fuel System */
        if(FaultStatusP->Faults.PFuel_High){ mexPrintf("\n PFuel High - at t = %f",t); }
        if(FaultStatusP->Faults.PFuel_Low){ mexPrintf("\n PFuel Low - at t = %f",t); }
        ...
        /* Mechanical failures */
        if(FaultStatusP->Faults.VibrationHigh){ mexPrintf("\n Vibration High - at t = %f",t); }
        if(FaultStatusP->Faults.Mec_Lock){ mexPrintf("\n Shaft Lock - at t = %f",t); }
        if(FaultStatusP->Faults.OverTemp){ mexPrintf("\n OverTemp - at t = %f",t); }
        if(FaultStatusP->Faults.OverSpeed){ mexPrintf("\n OverSpeed - at t = %f",t); }
        if(FaultStatusP->Faults.Ignition){ mexPrintf("\n Ignition - at t = %f",t); }
