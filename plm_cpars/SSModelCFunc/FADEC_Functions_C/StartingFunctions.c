...

{
	...
	/* Calculates the Value of Nc - requires definition of sqrtT */
	Nc = RPM / sqrtT(Theta);
	Nc = Nc / NDes;
...
void StartLogic(struct ControllerConstants *ContConstP,struct ControlStart *ContStartP, floating RPM, floating Tt4, digital_t *digitalP, floating Tsample)
{
	...
	int    IgnTries = ContStartP->IgnTries;
...
/*******************************************************************************
 *
 *	FUNCTION:		StartLogicTJ200()
 *
 *	PURPOSE:		Accomplishes the necessary fuel logic for TJ200 engine starging at
 *					test bench
 *
 *	CALLED FROM:	tbd
 *
...
void StartLogicTJ200(struct ControllerConstants *ContConstP,struct ControlStart *ContStartP, floating RPM, floating Tt4, digital_t *digitalP, floating Tsample)
{
	...
	case 1:
		// DigOutOn(RPM, ContConstP, digitalP, &status, 2);
		/* Starter motor speed command receives initial command */
		ContStartP->StrtRPMAct = ContConstP->StrtInit;
		status = 2;
		break;

	/* STATE 2 -> As soon as RPM reaches RPMRelaysOn, igniter is activated */
	case 2:
		// StatusDelay(&WaitTime, Tsample, ContConstP->WaitAction, &status, 3);
		/* Verifies if RPM is  larger or equal minimum value for ignition activation*/
		// printf("RPM=%f\n", RPM);
		if (RPM >= ContConstP->RPMRelaysOn)
		{
			/* Activates ignition */
			digitalP->bits.igniter = 1;
			// printf("igniter=%d\n", digitalP->bits.igniter);
			status = 21;
		}

		break;

	/* STATE 21 -> Waits igniter warming up time */
	case 21:
		// FuelPulse(&TimePulse, Tsample, PulseWidith, &Wf, ContConstP->WfHighPulse, Tt4, ContConstP->Tt4AvFlameOn, &status, 4);
		/* Wait for established time for ignition to warm up */
		StatusDelay(&WaitTime, Tsample, ContConstP->WaitAction, &status, 22);
		// printf("WaitTime=%f\n", WaitTime);
		break;

	/* STATE 22 -> Opens ignition solenoid valve */
	case 22:
		// IgnitAttempt(digitalP, &TimeIgn, ContConstP->TimeIgnition,
		// 			Tsample, Tt4, ContConstP->Tt4AvFlameOn, &status, 3,
		// 			&IgnTries,2, &Wf, ContConstP->WfInit,
		// 		 	&IgnitionFail, 41);
		/* Activates ignition solenoide valve */
		digitalP->bits.ignSOV = 1;
		status = 3;
		break;

	/* STATE 3 -> Sends initial fuel flow command */
	case 3:
		// StatusDelay(&TimeBeforeRamp, Tsample, ContConstP->TimeBeforeRamp, &status, 5);
		/* Gives first fuel flow pulse */
		FuelPulse(&TimePulse, Tsample, PulseWidith, &Wf, ContConstP->WfHighPulse, Tt4, ContConstP->Tt4AvFlameOn, &status, 31);
		break;

	/* STATE 31 -> Starts ignition attempts */
	case 31:
		...
		IgnitAttempt(digitalP, &TimeIgn, ContConstP->TimeIgnition,
					 ...
					 &IgnTries, 2, &Wf, ContConstP->WfInit,
					 &IgnitionFail, 4);
		// printf("IgnTries=%f IgnFail=%d\n",IgnTries,IgnitionFail);
		break;

	/* STATE 4 -> As soon as ignition takes place, main solenoid valve is open */
	case 4:
	...
	/* STATE 41 -> Executes starter ramp up to 100% */
	case 41:
		// AccomodSR(&TimeAccom, ContConstP->TimeAccomodation, Tsample, &StartComplete, &Wf, WfAccom);
		/* Accelerates electric starter using constant ramp */
		ContStartP->StrtRPMAct = ContStartP->StrtRPMAct + Tsample * ContConstP->StrtRampCoeff;
		/* Electric motor speed is limited to specified maximum value */
		if (ContStartP->StrtRPMAct >= ContConstP->StrtMax)
		{
			/* If maximum value is reached, speed command value is limited and status is updated */
			ContStartP->StrtRPMAct = ContConstP->StrtMax;
			status = 5;
		}
		break;

	/* STATE 5 -> Fuel ramp is executed */
	case 5:
		/* Fuel ramp execution */
		FuelIncrease(ContConstP, ContStartP, digitalP, RPM, ContConstP->RPMIdle, ContConstP->RPMIgnOff, Tt4,
					 ContConstP->Tt4AvFlameOn, &Wf, &WfAccom, WfChangeRamp, ContConstP->WfRamp,
	...
	/*------------------------------*/
	/* DEACTIVATON OF STARTER MOTOR */
	/*------------------------------*/

	/* If RPM lies above StrtRPMOff, the starter motor must be deactivated */
	...
	/*------------------------*/
	/* DEACTIVATON OF IGNITER */
	/*------------------------*/

	/* If RPM lies above RPMIgnOff, the igniter and its solenoid valve must be deactivated */
	if (RPM > ContConstP->RPMIgnOff)
	{
		digitalP->bits.igniter = 0;
		digitalP->bits.ignSOV = 0;
	}
...
void ManualKeysSR(digital_t *digitalP, keys_t *keysP, int EngineShutDown, floating *WfP)
{
	...
	digitalP->bits.igniter_led = 0;
	digitalP->bits.T4_alarm_led = 0;
	digitalP->bits.ignSOV = !keysP->bits.ignSOV_key;
