...
%Initiates AcMtx and BcMtx with corresponding SL matrices
% AcMtx = AMtx;
% BcMtx = BMtx;
% %Reads AcMtx dimensions
% [~,AcMtxCols] = size(AcMtx);
% 
% AcTest = AcMtx;
...
%         BcMtx(j,:,1) = BMtx(j,:,1) * sqrThetaSL;
    ILrow = DimX * (j - 1) + 1;
    IHrow = DimX * j;
    AcMtx(ILrow:IHrow,:,1) = AMtx(ILrow:IHrow,:,1) * sqrThetaSL / DeltaSL;
    BcMtx(ILrow:IHrow,:,1) = BMtx(ILrow:IHrow,:,1) * sqrThetaSL;
