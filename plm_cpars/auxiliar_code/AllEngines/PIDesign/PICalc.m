...
%Corrects the size considering space-variable dimension
line = line/DimX;
column = column/DimX;
...
for i = FirstIndex:length(AltLinVectorPT)
    for j=1:length(MNVectorPT)
        ...
        ILrow = DimX * (j - 1) + 1;
        IHrow = DimX * j;
        AMtx(ILrow:IHrow,:,i) = AcMtx(ILrow:IHrow,:,1) * DeltaMtx(i,j) / sqrThetaMtx(i,j);
        BMtx(ILrow:IHrow,:,i) = BcMtx(ILrow:IHrow,:,1) / sqrThetaMtx(i,j);
...
%             dentf = [1 -AMtx(j,k,i)];
            ...
            A = AMtx(ILrow:IHrow,ILcol:IHcol,i);
            C = NCMtx(j,ILcol:IHcol,1);
            
            ILcol = DimU * (k - 1) + 1;
            IHcol = DimU * k;
            
            B = BMtx(ILrow:IHrow,ILcol:IHcol,i);
            D = NDMtx(j,ILcol:IHcol,1);
