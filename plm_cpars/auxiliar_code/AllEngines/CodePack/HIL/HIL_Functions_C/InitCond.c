...

{
	...
	floating Tt2, Pt2, Theta, Delta, ThetaSL, DeltaSL;
	floating AltLinVec = 0;
...
printf("FNpc0=%f\n",FNpc0);
	...
	if (Index3D.fRPM < 0) {
		...
		*(SkipStartP) = 0;
		...
		/* Finds the absolute value of RPM0 for the corresponding altitude - requires definition of sqrtT */
		RPM0 = RPM0 * NDes * sqrtT(Theta);

		/* Finds the value of Wf0 from Sea Level Matrix */
		Wf0 = InterpValue(&Index3D, WfMtxP, Mtx2DP, Mtx2DHP, Mtx1DP, Mtx1DHP, NumEl2D, NumEl1D);
		/* Calculates the corrected value of Wf0 - requires definition of sqrtT */
		Wf0 = Wf0 / DeltaSL / sqrtT(ThetaSL);
		/* Calculates the absolute value of Wf0 for the corresponding altitude - requires definition of sqrtT */
		Wf0 = Wf0 * Delta * sqrtT(Theta);
