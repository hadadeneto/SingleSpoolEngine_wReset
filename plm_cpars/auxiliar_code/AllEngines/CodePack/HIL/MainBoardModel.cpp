...
//printf("model_mf=%f\n", model_mf);
        ...
        {								//TICK value received from FADEC, used on HIL Sync Mode. Informs the model the interval of the simulation in miliseconds
            ...
            if(simMod == 2)
            {
                Ts = 0.001 * tickFromFADEC;
                TsamModel = Ts / TsampDiv;
            }
...
//        }
        ...
        if((HILMode == 0) | (HILMode == 1))  //0 = SIL, 1 = HIL Async, 2 = HIL Sync, 3 = Test Bench. So, if the mode is SIL or HIL Async, starts the periodic_timer.
        {
            ...
            Ts = 0.001 * TICK;
            TsamModel = Ts / TsampDiv;
                    //periodic_timer.startPeriodic(uavcan::MonotonicDuration::fromMSec(Ts*1000));	//PERIODO DE ATUALIZACAO DOS DADOS NO BARRAMENTO
                    //upTime_ms = 0;
                    //lastUpdateTime_ms = 0;
        }
...
//printf("Executing Engine Model Loop\n");
// printf("Wf=%f FuelIn=%f\n",Wf,FuelIn);
...
/******** COPYFROM END: ENGINEMODEL ********/
	...
	if(bit_start == 0 & (upTime_ms > (Ts*2000)))
	{
