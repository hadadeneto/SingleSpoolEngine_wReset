...
void PIMinMaxTF1200(struct ControlStart *ContStartP, struct ControllerInput *ContInputP, struct ControllerOutput *ContOutputP, unsigned int *digitalP, unsigned int *keysP, double *NcFANContMtxP, double *FNperContMtxP, double *N_KpMtxP, double *N_KiMtxP, double Tsample)
{
	...
	double KiN;
	double KpN;
	double KbN = TF1200_KBN;
	double KbT = TF1200_KBT;
	double KpTt4 = TF1200_KPTT4;
	double KiTt4 = TF1200_KITT4;
	...
	double Mtx2D[TF1200_MN_ELEM * TF1200_FN_ELEM];
	double Mtx2DH[TF1200_MN_ELEM * TF1200_FN_ELEM];
	double Mtx1D[TF1200_FN_ELEM];
	double Mtx1DH[TF1200_FN_ELEM];
	...
	double NcVec[] = { 0.6951, 0.7417, 0.8, 0.862, 0.912, 0.98 };
	double ASVec[] = { 1.0085, 1.0112, 1.01875, 1.044, 1.074, 1.02041 };

	double ASRef;
	double MinRef;

	double NumEl2D = TF1200_MN_ELEM * TF1200_FN_ELEM;
	double NumEl1D = TF1200_FN_ELEM;
	...
	else
	{
		...
		else
		{
			...
			if(LeverMode == TF1200_CLOOP)
			{
				...
				InterpInd3DRPM(Altitude, MN, FNpc, &AltLinVec, &MNLinVec, FNperContMtxP, &Mtx2D, &Mtx2DH, &Mtx1D, &Mtx1DH, TF1200_ALT_ELEM, TF1200_MN_ELEM, TF1200_FN_ELEM, &Index3D);
				RPMRef = InterpValue(&Index3D, NcFANContMtxP, &Mtx2D, &Mtx2DH, &Mtx1D, &Mtx1DH, NumEl2D, NumEl1D);
				RPMRef = RPMRef * TF1200_NDES*sqrt(Theta);

				/* REFERENCE FILTER */

				/* Calculates the Acceleration Schedule Value for current Nc */
				ASRef = ASRefFind(&NcVec, &ASVec, Nc, Tt2, TF1200_AS_ELEM, TSTD, TF1200_NDES);

				/* Calculates the minimum value between ASRef and NMAX */
				MinRef = Min(ASRef, TF1200_NMAX);
				...
				RefRamp(&MinRef, &RefInit, &RefRampReset, TF1200_REFRAMP, TICK);

				/* CALCULATION OF KpN AND KiN */

				/* Finds 3D indexes necessary for interpolation of controller gains */
				InterpInd3DRPM(Altitude, MN, Nc, &AltLinVec, &MNLinVec, NcFANContMtxP, &Mtx2D, &Mtx2DH, &Mtx1D, &Mtx1DH, TF1200_ALT_ELEM, TF1200_MN_ELEM, TF1200_FN_ELEM, &Index3D);
				...
				eNk = MinRef - RPM;
				//eTt4k = TF1200_TT4MAX - Tt4;
				eTt4k = TF1200_TT5MAX - Tt5;
				eBNk1 = uNk1 - urk1;
				eBTk1 = uTt4k1 - urk1;
			...
			else
			{
				...
				OpenLoop(ContStartP, FuelFlow, &fValue, EngineShutDown, &Reset);
