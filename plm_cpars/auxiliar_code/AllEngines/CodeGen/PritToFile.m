% function PritToFile(Alt,dTValue,Nc,MN,A,B,C,D,KpNP,KiNP,...
...
%     FNperCont,NcFANCont,EngineName)
    ...
    function ExtFolders = PritToFile(Ts0,Delta,Theta,A,B,...
    ...
    FNperCont,FNcMaxCont,NcFANCont,EngineName,...
    MNLinVec,AltLinVec,...
    Awm,Bwm,Cwm,Dwm,RPMwm,...
    Ash,Bsh,Pt3Csh,Pt3Dsh,Tt4Csh,Tt4Dsh,...
    NcFANsh,Wfsh,Tt4sh,Pt3sh,RPMsh,...
    ASVec,DSVec,NcVec,...
    FuelPumpIn,FuelPumpOut,...
    OilPumpIn,OilPumpOut,EngFoldName)
...
%   Detailed explanation goes here
    ...
    %Initiates File_Sep with empty value
    File_Sep = [];
    CCodeFoldName = [];
    %Allocates filesep to File_Sep if EngFoldName is not empty
    if ~isempty(EngFoldName)
        File_Sep = filesep;
        CCodeFoldName = EngineName;
    end
    
    %Folder for code files
    ExtFolders = [EngFoldName File_Sep CCodeFoldName File_Sep ];
    
    %Creates a folders for organizing files
    cFolderName = [ExtFolders 'c_Files'];
    hFolderName = [ExtFolders 'h_Files'];    
...
%     NumElWord = num2str(rows*columns*pages);
    ...
    Prt3DCMtx(EngineName,'MNLinVec',MNLinVec,num2str(length(MNLinVec)),ArgType,ArgName,RetType,cFolderName);
    Prt3DCMtx(EngineName,'FNcMaxContMtx',FNcMaxCont,num2str(length(MNLinVec)),ArgType,ArgName,RetType,cFolderName);
    
    %Cold Sub-Idle matrices
    Prt3DCMtx(EngineName,'AwmMtx',Awm,num2str(length(Awm)),ArgType,ArgName,RetType,cFolderName);
    Prt3DCMtx(EngineName,'BwmMtx',Bwm,num2str(length(Bwm)),ArgType,ArgName,RetType,cFolderName);
    Prt3DCMtx(EngineName,'CwmMtx',Cwm,num2str(length(Cwm)),ArgType,ArgName,RetType,cFolderName);
    Prt3DCMtx(EngineName,'DwmMtx',Dwm,num2str(length(Dwm)),ArgType,ArgName,RetType,cFolderName);
    Prt3DCMtx(EngineName,'RPMwmMtx',RPMwm,num2str(length(RPMwm)),ArgType,ArgName,RetType,cFolderName);
