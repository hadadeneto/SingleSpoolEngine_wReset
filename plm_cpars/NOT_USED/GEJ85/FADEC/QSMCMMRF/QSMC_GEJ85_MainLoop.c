...
#define S_FUNCTION_NAME  QSMC_GEJ85_MainLoop
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    real_T       *QSMC_out = ssGetOutputPortSignal(S, 1);
	...
	/* MATRICES USED BY ENGINE QSMC CONTROLLER */

    /* Delta and theta matrices */
    double ThetaMtx[GEJ85_MN_ELEM * GEJ85_ALT_ELEM]; 
    double DeltaMtx[GEJ85_MN_ELEM * GEJ85_ALT_ELEM]; 
    
    /* State-space matrices */
    double AMtx[GEJ85_MN_ELEM * GEJ85_FN_ELEM]; 
    double BMtx[GEJ85_MN_ELEM * GEJ85_FN_ELEM]; 
    double CTMtx[GEJ85_MN_ELEM * GEJ85_FN_ELEM]; 
    double DTMtx[GEJ85_MN_ELEM * GEJ85_FN_ELEM]; 
    
    /* Steady variables matrices */
    double Tt5Mtx[GEJ85_MN_ELEM * GEJ85_FN_ELEM]; 
    double WfMtx[GEJ85_MN_ELEM * GEJ85_FN_ELEM]; 
    
    /* Pointer used by QSMC controller */
    QSMCInput QSMCInPx;
	QSMCInput QSMCInPy;
    ...
    getGEJ85ThetaMtx(&ThetaMtx);
    
    /* State-space parameters */
	getGEJ85AMtx(&AMtx);
	getGEJ85BMtx(&BMtx);
    
    /* Observed variable paraeters */
    getGEJ85Tt5CMtx(&CTMtx);
	getGEJ85Tt5DMtx(&DTMtx);
	...
	/* Reads Pt2 - Only needed for QSMC */
	Pt2 = InputVars[14];
    ...
    if(Authorize == 1)
    {
        ...
        //QSMCMinMaxGEJ85(&ContStart, &ContInput, &ContOutput, &digitalWord, &keysWord, &NcFANContMtx, &FNperContMtx,
        //                &ThetaMtx, &DeltaMtx, &AMtx, &BMtx, &CTMtx, &DTMtx, &Tt5Mtx, &WfMtx, &QSMCIn);
		QSMCMinMaxGEJ85(&ContStart, &ContInput, &ContOutput, &digitalWord, &keysWord, &NcFANContMtx, &FNperContMtx,
						&ThetaMtx, &DeltaMtx, &AMtx, &BMtx, &CTMtx, &DTMtx, &Tt5Mtx, &WfMtx, Ts);
	...
	QSMC_out[0] = ContOutput.QSMC_dx;
	QSMC_out[1] = ContOutput.QSMC_dy;
	QSMC_out[2] = ContOutput.QSMC_ux;
	QSMC_out[3] = ContOutput.QSMC_uy;
