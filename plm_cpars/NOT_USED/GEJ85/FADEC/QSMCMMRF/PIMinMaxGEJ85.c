...
void PIMinMaxGEJ85(struct ControlStart *ContStartP, struct ControllerInput *ContInputP, struct ControllerOutput *ContOutputP, unsigned int *digitalP, unsigned int *keysP, double *NcFANContMtxP, double *FNperContMtxP, double *N_KpMtxP, double *N_KiMtxP)
{
	...
	POilLogic(ContStartP, POil, &OilPresSRP, &digital, EngineShutDown);
	...
	else
	{
		if (StartComplete == 0)
		{
			...
			FuelLogic(ContStartP, RPM, Tt5, &FuelSRP, &digital, EngineShutDown);
