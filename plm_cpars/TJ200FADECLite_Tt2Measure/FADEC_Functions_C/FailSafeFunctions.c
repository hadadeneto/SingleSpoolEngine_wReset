...
#include "../FADEC_Functions_H/PIOil.h"
...
//    }StatusUpdate;
    ...
    floating TimerWindMill = ContStartP->TimerWindMill;
    floating TimerReadytoStart = ContStartP->TimerReadytoStart;
    floating TimeWindMill = ContConstP->TimeWindmill;
    floating TimeReadytoStart = ContConstP->TimeReadytoStart;
 ...
 *	CALLED FROM:	tbd
 *
 *	PARAMETERS:		struct ControllerInput *ContInputP, floating reference, floating *FNpc
 *
 *	RETURNS:		void
 *
 *	COMMENTS:
...
void BoosterProtect(struct ControllerConstants *ContConstP, struct ControllerInput *ContInputP, floating Delta, int BoosterLStatus, int BoosterRStatus, floating Ts)
{
    ...
    floating TimerBooster = ContInputP->TimerBoost;
                    ...
                    floating *FNperContMtxP, struct ControllerConstants *ContConstP, struct InterpIndex3D *Index3DP)
{
    ...
    floating Altitude = ModelParsP->Altitude;
    floating MN = ModelParsP->MN;
    floating FNpc = ModelParsP->FNpc;
    
    /* Elements numbers */
    floating Alt_Elem = ContConstP->Alt_Elem;
    floating MN_Elem = ContConstP->MN_Elem;
    floating Fn_Elem = ContConstP->Fn_Elem;
...
int RelError(floating Sens0, floating Sens1, floating MaxError)
{
    ...
    floating LargeSens = Sens0;
    floating SmallSens = Sens1;
    floating RelativeError;
    floating NearZero = 1e-10;
    int SensorMatch = 0;
 ...
 *	CALLED FROM:	tbd
 *
 *	PARAMETERS:		floating *SensVec, floating *MatchVec, floating NumSens, floating MaxError
 *
 *	RETURNS:		void
 *
 *	COMMENTS:       
...
void Ts2MediumValue(ModelParsFault *ModPFaultP, floating *Ts2MedValP, int SensNum, FaultDetect *TrustSensP, struct SensedPars *SensorsP, floating Ts2Model,  int MinTrustedSens)
{
    ...
    int NumTrusted = 0;
    int UseModVal = 0;
    floating SensorAux[5] = {0,0,0,0,0};
...
// void VarMediumValue(int VarPos, ModelParsFault *ModPFaultP, floating *MedValP, int SensNum, uint64_t TrustSens, floating *SensorsP, floating ModelVal,  int MinTrustedSens)
// {
//     int i;
//     int TruncBits = 0;
//     int TrustBit = 1;
...
// void ResidualCompare(floating ResMain, floating ResComp, int *CounterP, int MaxValue, EngineFaults *FaultStatusP, EngineFaults *FaultWarningP,  int NumShifts)
// {
...
// void FaultIdent(floating ResModel, floating Threshold, floating ResComp, int *CounterP, int MaxValue, EngineFaults *FaultStatusP, EngineFaults *FaultWarningP,  int NumShifts)
// {
// // printf("\n\nResModel = %f; ",ResModel);
...
void OverTemp(struct ControllerConstants *ContConstP, FaultDetect *TrustSensP, SystemFaults *FaultSSysP, struct SensedPars *SensorsP, EngineFaults *FaultStatusP)
{
    ...
    int CounterOT = 0;
    int CounterNOT = 0;
    floating SensVecOT[3] = {0,0,0};
    floating SensVecNOT[3] = {0,0,0};
    int MatchVec[3] = {0,0,0};
    floating MaxError = ContConstP->TCThreshold;
    // floating MaxError = TJ1200_TCTHRESHOLD;
    //int MatchElem = 0;
...
void OverSpeed(FaultDetect *TrustSensP, SystemFaults *FaultSSysP, struct SensedPars *SensorsP, EngineFaults *FaultStatusP)
{
    ...
    int CounterOS = 0;
    int CounterNOS = 0;
    floating SensVecOS[2] = {0,0};
    floating SensVecNOS[2] = {0,0};
...
//    floating MaxInter_POilHigh = ContConstP->TimeHighPOil;
//    floating MaxInter_POilLow = ContConstP->TimeLowPOil;
//    floating MaxInter_PFuelHigh = ContConstP->TimeHighPFuel;
//    floating MaxInter_PFuelLow = ContConstP->TimeLowPFuel;
//    floating MaxInter_VibHigh = ContConstP->TimeHighVibration;
//    floating MaxInter_Overspeed = ContConstP->TimeOverspeed;
//    floating MaxInter_OverTemp = ContConstP->TimeOvertemp;
...
//    MaxInter_OverTemp = MaxInter_OverTemp / TICK / 0.001;
    ...
    floating PFuelShort = ContConstP->PFuelShort;
//    floating PSOpen = ContConstP->PSOpen;
    floating PSOpenThresh = ContConstP->PSOpenThresh; 
    floating PSShort = ContConstP->PSShort; 
    floating PSShortThresh =  ContConstP->PSShortThresh;
//    floating VibOpen = ContConstP->VibOpen;
//    floating VibOpenThresh = ContConstP->VibOpenThresh;
    floating VibShort = ContConstP->VibShort;
    floating VibShortThresh = ContConstP->VibShortThresh;

    /* Thresholds */
    floating TCThreshold = ContConstP->TCThreshold; 
    floating Tt4ShaftLock = ContConstP->Tt4ShaftLock;
    floating PSThreshold =  ContConstP->PSThreshold;
    floating VibThreshold = ContConstP->VibThreshold;
    floating RPMThreshold = ContConstP->RPMThreshold;

    /* Limit Values */
    floating PFuelHigh = ContConstP->PFuelHigh; 
    floating PFuelLow = ContConstP->PFuelLow;
    floating POilHigh = ContConstP->POilHigh; 
    floating POilLow = ContConstP->POilLow;
    floating VibHigh = ContConstP->VibHigh;
    floating Tt4Max = ContConstP->Tt4Max;
    floating NMax = ContConstP->NMax;
...

{
    ...
    /* Used internal variables */
    floating Ts2Model = ModelParsP->Ts2;
    int i;
...
//    int Alt_Elem = ContConstP->Alt_Elem;
    ...
    /* Short-Open sensor values */
    floating PFuelShort = ContConstP->PFuelShort;
    floating PFuelOpen = ContConstP->PFuelOpen;
    floating POilShort = ContConstP->POilShort;
    floating POilOpen = ContConstP->POilOpen;
    floating PSOpen = ContConstP->PSOpen;
//    floating PSOpenThresh = ContConstP->PSOpenThresh;
    floating PSShort = ContConstP->PSShort; 
//    floating PSShortThresh =  ContConstP->PSShortThresh;
...
//    floating VibOpenThresh = ContConstP->VibOpenThresh;
    floating VibShort = ContConstP->VibShort;
//    floating VibShortThresh = ContConstP->VibShortThresh;
    floating RPMShort = ContConstP->RPMShort;
    floating RPMOpen = ContConstP->RPMOpen;
    floating TCOpen = ContConstP->TCOpen;
...
//    int NumTs2Sens = ContConstP->NumTs2Sens;
    ...
    floating TCThreshold = ContConstP->TCThreshold; 
    floating Tt4ShaftLock = ContConstP->Tt4ShaftLock;
    floating Tt4BlowOut = ContConstP->Tt4BlowOut;
    floating PSThreshold =  ContConstP->PSThreshold;
    floating Pt3BlowOut = ContConstP->Pt3BlowOut;
//    floating VibThreshold = ContConstP->VibThreshold;
//    floating RPMThreshold = ContConstP->RPMThreshold;
    floating RPMBlowOut = ContConstP->RPMBlowOut;
...

{
    ...
    /* Thresholds */
    floating TCThreshold = ContConstP->TCThreshold;
    floating Ts2Threshold = ContConstP->Ts2Threshold;
    floating Tt4Threshold = ContConstP->Tt4Threshold; 
    floating PSThreshold =  ContConstP->PSThreshold;
    floating Pt3Threshold = ContConstP->Pt3Threshold;
    floating VibThreshold = ContConstP->VibThreshold;
    floating RPMThreshold = ContConstP->RPMThreshold;
    floating POilThreshold = ContConstP->POilThreshold;
    floating PFuelThreshold = ContConstP->PFuelThreshold;
 ...
 *	CALLED FROM:	tbd
 *
 *	PARAMETERS:		floating Ts2, struct EstimatedPars *ModelParsP, struct InterpIndex3D *Index3DP
 *
 *	RETURNS:		None
 *
 *	COMMENTS:
...

{
    ...
    floating dT;   
    floating gamma = ModelParsP->gamma_air;
    floating MN = ModelParsP->MN;
    floating TRatio;
    floating Ts2Std = ModelParsP->Ts2Std;
    floating Tt2;   
    floating Tt2StdSL;
...
//	floating X_N_TVec[20] = {300, 10000};
    ...
    /*--------Define Constants-------*/
    floating choked, Ts, rhos, V, Ptin;// Test, MN1, Ptin;
    floating CdTh, Cv, Cfg;//, Therm_growth;
    floating PQPa, PQPaMap;//, AthroatHot;
    floating Rt, TsMN1, PsMN1;//, Woutcalc;
    floating WOut, FgOut;//, NErrorOut;
    floating Ath, Vth, Psth;//, Ax, Vx, Psx, Tsx, gammasx, MNx;
                 ...
                 floating *MapHPC1DP, floating *MapHPC1DHP, floating *MapHPC2DP, floating *MapHPC2DHP)
{
    ...
    /* Air constants */
    floating gamma_air = 1.4;
    floating R_air = 287;
    
    /* Number of integration steps */
    floating NumSteps = ModelParsP->NumSteps;
    
    /* Sampling Time */
    floating Ts = (ModelParsP->Ts) / NumSteps;
    
    /* Ambient */
    floating Altitude = ModelParsP->Altitude;
    floating MN = ModelParsP->MN;
//    floating dT = ModelParsP->dT;
    floating VSound;
    floating VFlight;

	/* Station 2 */
    floating Pt2;
    floating Ps2 = ModelParsP->Ps2;
    floating Ts2 = ModelParsP->Ts2;
    floating Ts2Sensed = ModelParsP->Ts2Sensed;
    floating Tt2 = ModelParsP->Tt2;
	floating Tt2SL = ModelParsP->Tt2StdSL;
	floating N2c;
    floating WcHPC;
    floating W2;
    ...
    floating PRHPC;
	floating Pt3;
    floating Pt3Sensed = ModelParsP->Pt3Sensed;
	/* Station 4 */
	floating Tt4;
    floating Tt4Sensed = ModelParsP->Tt4Sensed;
    floating Wf;                              /* Variable used for model estimation */
    floating WfIn = ModelParsP->Wf;           /* Controller signal */
    floating WfOutfp = ModelParsP->WfOutfp;
    /* Station 5 */
    floating W5;
    floating Pt5;
    floating Tt5;
    /* Station 8 */
    floating Ath = ContConstP->Ath;
    floating MFP;
	/* Shaft design rotation speed */
	floating RPM = ModelParsP->RPM; 
    floating NDes = ContConstP->NDes;
    // floating NDes = TJ1200_NDES;
	...
	floating A;
	floating Attc = ContConstP->TTCTimeConst;
	floating Apsens = ContConstP->PSensTimeConst;
	floating Afp = ContConstP->FuelPumpTimeConstant;
	floating B;
	floating Bttc = -1 * ContConstP->TTCTimeConst;
	floating Bpsens = -1 * ContConstP->PSensTimeConst;
	floating Bfp = -1 * ContConstP->FuelPumpTimeConstant;
	floating Ad;
	floating Adttc;
	floating Adpsens;
	floating Adfp;
...
//printf("%f\n", Tt4);
        ...
        W5 = W2 + Wf;    
        /* Calculation of MFP - requires definition of sqrtT */
        MFP = W5 * sqrtT(Tt5) / Pt5 / Ath;
...
//printf("Tt4Sensed = %f + %f * (%f - %f)\n", Tt4Sensed, Bdttc, Tt4Sensed, Tt4);
        ...
        VSound = sqrtT(gamma_air * R_air * Ts2);
        VFlight = VSound * MN;
