...
figure(1)
hold on
plot(t(FirstInst:end),N_HIL(FirstInst:end)/NDes/NcInit,'linewidth',2)
...
%Plots the Rotation Speed on top of reference if closed-loop
plot(t(FirstInst:end),RPMRef_HIL(FirstInst:end)/NDes/NcInit,':','linewidth',1)
plot(t(FirstInst:end),FiltRef_HIL(FirstInst:end)/NDes/NcInit,'-','linewidth',1)
...
% figure(18)
% hold on
% plot(t(FirstInst:end),Active_FADEC(FirstInst:end),'linewidth',2)
% xlabel('Time (s)', 'FontSize', 24);
